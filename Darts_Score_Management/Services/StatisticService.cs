...
namespace Darts_Score_Management.Services
{
    public class StatisticService : IStatisticService
    {
        ...
        public async Task UpdateLegStatsAsync(int legId, List<GamePlayer> gamePlayers)
        {
            ...
            {
                int currentPlayerId = gamePlayer.PlayerId;
                IEnumerable<Turn> turns = await _turnService.GetPlayerTurnsInLegAsync(currentPlayerId, legId);
                LegStats legStats = CalculateLegStats(turns, gamePlayer.Id, legId);
        ...
        {
            List<GamePlayer> gamePlayers = await GetGamePlayersForSetAsync(setId);
   
            foreach (var gp in gamePlayers)
            {
                IEnumerable<LegStats> playerLegStats = await _legStatsRepository.GetLegStatsForPlayerInSetAsync(setId, gp.Id, gp.PlayerId);
                SetStats setStats = AggregateSetStats(playerLegStats, gp.Id, setId, legsWonPerPlayer);
        ...
        {
            List<GamePlayer> gamePlayers = await GetGamePlayersForGameAsync(gameId);
           
            foreach (GamePlayer gp in gamePlayers)
            {
                IEnumerable<SetStats> playerSetStats = await _setStatsRepository.GetSetStatsForPlayerInGameAsync(gameId, gp.Id, gp.PlayerId);
                GameStats gameStats = AggregateGameStats(playerSetStats, gp.Id, gameId, setsWonPerPlayer);
        ...
        private async Task<List<GamePlayer>> GetGamePlayersForSetAsync(int setId)
        {
            ...
            List<GamePlayer> gamePlayers = set.Game.GamePlayers.ToList();    
            return gamePlayers;
        }
        ...
        {
            decimal ppd = StatisticCalculator.CalculatePPD(turns);
            decimal first9PPD = StatisticCalculator.CalculateFirst9PPD(turns);
            int totalThrows = turns.Sum(t => t.Throws.Count);
            int checkoutPercentage = StatisticCalculator.CalculateCheckoutPercentage(turns);
            int count60Plus = StatisticCalculator.CalculateCount60Plus(turns);
            int count100Plus = StatisticCalculator.CalculateCount100Plus(turns);
            int count140Plus = StatisticCalculator.CalculateCount140Plus(turns);
            int count180s = StatisticCalculator.CalculateCount180s(turns);
        ...
        {
            int totalThrows = legStats.Sum(ls => ls.TotalThrows);
            decimal ppd = StatisticCalculator.CalculatePPDAggregate(legStats);
            decimal first9PPD = StatisticCalculator.CalculateFirst9PPDAggregate(legStats);
            GamePlayer gamePlayer = _context.GamePlayers.FirstOrDefault(gp => gp.Id == gamePlayerId);
            int playerId = gamePlayer?.PlayerId ?? 0;
            int legsWon = legsPerPlayer.ContainsKey(playerId) ? legsPerPlayer[playerId] : 0;
            var playerTurns = legStats.SelectMany(ls => _context.Turns
            .Where(t => t.LegId == ls.LegId && t.PlayerId == playerId));
            int checkoutPercentage = StatisticCalculator.CalculateCheckoutPercentage(playerTurns);
            int count60Plus = legStats.Sum(ls => ls.Count60Plus);
            int count100Plus = legStats.Sum(ls => ls.Count100Plus);
            int count140Plus = legStats.Sum(ls => ls.Count140Plus);
            int count180s = legStats.Sum(ls => ls.Count180s);
        ...
        {
            int totalThrows = setStats.Sum(ss => ss.TotalThrows);
            decimal ppd = StatisticCalculator.CalculatePPDAggregate(setStats);
            decimal first9PPD = StatisticCalculator.CalculateFirst9PPDAggregate(setStats);
            GamePlayer gamePlayer = _context.GamePlayers.FirstOrDefault(gp => gp.Id == gamePlayerId);
            int playerId = gamePlayer?.PlayerId ?? 0;
            int setsWon = setsWonPerPlayer.ContainsKey(playerId) ? setsWonPerPlayer[playerId] : 0;
            int legsWon = setStats.Sum(ss => ss.LegsWin);
            var playerTurns = setStats.SelectMany(ss => _context.Turns
            .Where(t => t.Leg.SetId == ss.SetId && t.PlayerId == playerId));
            int checkoutPercentage = StatisticCalculator.CalculateCheckoutPercentage(playerTurns);
            int count60Plus = setStats.Sum(ss => ss.Count60Plus);
            int count100Plus = setStats.Sum(ss => ss.Count100Plus);
            int count140Plus = setStats.Sum(ss => ss.Count140Plus);
            int count180s = setStats.Sum(ss => ss.Count180s);
        ...
        private PlayerStatsDTO CalculatePlayerStats(IEnumerable<LegStats> last10LegStats, IEnumerable<GamePlayer> gamePlayers)
        {
            ...
            List<LegStats> allLegStats = gamePlayers.SelectMany(gp => gp.LegStats ?? Enumerable.Empty<LegStats>()).ToList();
            List<LegStats> allLegStatsCombined = allLegStats.Concat(last10LegStats)
                .DistinctBy(ls => ls.Id) 
                .ToList();
            int totalLegsPlayed = allLegStatsCombined.Count;
            int legsWon = allLegStatsCombined.Count(ls => ls.Leg?.WinnerPlayerId == playerId);
            int legsLost = totalLegsPlayed - legsWon;

            Last10StatsDTO last10Stats = new Last10StatsDTO();
